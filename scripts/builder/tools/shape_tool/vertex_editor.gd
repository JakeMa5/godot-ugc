...
signal completed_transaction(data: Array)

## Emitted when the player cancels the tool transaction.
signal cancelled_transaction(data: Array)
...
func _init(poly: Polygon2D) -> void:
		self._vertices = poly.polygon
		self.scale = poly.scale
...
func _draw():
	for i in range(_vertices.size()):
		...
		if i == _hovered_vertex:
			draw_circle(v, 8, Color(1, 1, 1), true, -1.0, true)
			draw_circle(v, 5, Color.DARK_SEA_GREEN, true, -1.0, true)
		else:
			draw_circle(v, 6, Color(1, 1, 1), true, -1.0, true)
...
func _input(event):
	...
	if event is InputEventMouseButton:
		...
		if event.is_action_pressed("ui_cancel"):
			completed_transaction.emit(_vertices)
			queue_free()
